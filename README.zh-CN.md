...
当前验证出的正确配置是：保留宿主 VPN、模拟器 hosts 只保留 localhost、`version.db` 固定为 `type 0=55` 和 `type 1=8`，启动方式使用 `nohup`，避免堆积大量 Terminal 窗口。

在测试 Mac 上，Android Emulator 36.5.11 进入 3D 场景时仍可能出现图形后端错误，例如 `bad window surface handle` 或 `bad color buffer handle`。这属于模拟器图形后端稳定性问题，诊断脚本会提取这些错误，方便后续继续定位。
